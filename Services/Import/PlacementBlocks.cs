...
// Shared parser for the block layout used by publisher monthly results workbooks.
// The sheet is a column of placements: a brand/metric HEADER
...
internal static class PlacementBlocks
{
        ...
        string? audienceHint = null)
    {
        ...
        while (row <= lastRow)
        {
            ...
            row = ParsePlacement(ws, ctx, doc, row, brandCol, currentBrand ?? "", metrics, lastRow, lastCol, audienceHint);
        }
    }
        ...
        string? audienceHint)
    {
        ...
        var placement = new ParsedPlacement
        {
            ...
            Publisher = "",
            Template = ctx.Vocabulary.InferTemplate(name, keys),
            Name = name,
        ...
        // A text row with no month rows under it is a footnote, not a placement.
        if (placement.Actuals.Count > 0)
        {
    ...
    private static List<(int Col, string Key, string Label)> ReadMetrics(IXLWorksheet ws, int row, int metricStart, int lastCol)
    {
        ...
        for (int c = metricStart; c <= lastCol; c++)
        {
            ...
            var key = MetricNaming.Normalize(label);
